...
(function (W, D) {
	...
	function createWorker(code) {
		var blobURL = URL.createObjectURL(new Blob(['(', code.toString(), ')()'], {
	...
	var mainWk = createWorker(function () {
		...
		Blowfish.prototype = {
			...
			}, // | 0 приводит к 32битному значению
	...
	function addTrack(track) {
		...
		delEl.onclick = function () {
			window.dzDL.deleteCustom(this.parentElement);
		};
		delEl.innerHTML = '[X]';
	...
	elCombo.onclick = function () {
		window.dzDL.dlFlags = parseInt(this.value);
	};
	...
	var dzDL = {
		...
		FLAC: fmtFLAC,
		LOSSLESS: fmtFLAC,
		WITH_TITLE: flagTitle,
		WITH_ART: flagArt,
		WITH_HASH: flagHash,
		WITH_VER: flagVer,
		WITH_FMT: flagFmt,
		NAME_DEFAULT: /*flagTitle |*/ flagArt,
		NAME_DB: flagHash | flagVer,

		// Configuration
		dlFlags: flagHash | flagVer | flagTitle,
		// Miscellaneous functions
		hasTrackInDB: hasTrack,
		getTrackFromDB: getTrack,
		hasDownloadedTrack: hasDownloaded,
		getDownloadedTrack: getDownloaded,
		generateFileName: generateName,
		// Entry points
		getFromURL: decryptURL,
		makeURL: encryptURL,
		download: dzDownload,
		deleteCustom: deleteElement,
		addCustom: addTrack,
