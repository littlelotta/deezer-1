export class Blowfish {
	...
	decryptCBC(data: any, iv: number[]) {
		var blocks = Math.ceil(data.length / 8);
		var ivL, ivR, ivLtmp, ivRtmp, ivLivR;
		ivLivR = this.split64by32(iv);
		ivL = ivLivR[0];
		ivR = ivLivR[1];
		var decrypted: number[] = [];
		for (var i = 0; i < blocks; i++) {
			var block = data.slice(i * 8, (i + 1) * 8);
	...
	encipher(xL: number, xR: number): number[] {
		var tmp;
		for (var i = 0; i < 16; i++) {
	...
	decipher(xL: number, xR: number): number[] {
		var tmp;
		xL = this.xor(xL, this.pArray[17]);
		xR = this.xor(xR, this.pArray[16]);
		tmp = xL;
	...
	fixNegative(number: number): number {
		return (number >>> 0)
	}

	split64by32(block64: number[]): number[] {
